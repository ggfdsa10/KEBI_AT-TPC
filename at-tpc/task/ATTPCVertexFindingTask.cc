#include "ATTPCVertexFindingTask.hh"
...
ClassImp(ATTPCVertexFindingTask)

ATTPCVertexFindingTask::ATTPCVertexFindingTask()
:KBTask("ATTPCVertexFindingTask","")
{
}

bool ATTPCVertexFindingTask::Init()
{
...
void ATTPCVertexFindingTask::Exec(Option_t*)
{
    ...
    // kb_debug << " ================================ sTest: " << sTest << "  kAtSLeast: " << kAtSLeast << endl; 
    KBVector3 testPosition(fReferenceAxis, 0, 0, kAtSLeast);
    sTest = TestVertexAtK(vertex, testPosition, true);
    // kb_debug << "  X: " << vertex -> GetPosition().X() << "  Y: " << vertex -> GetPosition().Y() << "  Z: " << vertex -> GetPosition().Z() << endl;
...
Double_t ATTPCVertexFindingTask::TestVertexAtK(KBVertex *vertex, KBVector3 testPosition, bool last)
{
    Double_t sTest = 0;
    ...
    for (Int_t iTrack = 0; iTrack < numTracks; iTrack++) {
        ...
        // kb_debug << " testPosition: " << testPosition.X() << " " << testPosition.Y() << " "<< testPosition.Z() << " || xyzOnHelix:" << xyzOnHelix.X() << " " << xyzOnHelix.Y() << " " << xyzOnHelix.Z() << endl;
        // kb_debug << " iTrack : " << iTrack << " dist : " << dist << endl;
        if (last && dist > 10)
            continue;
    ...
    // kb_debug << " numUsedTracks" << numUsedTracks << "   averagePositionXYZ: " << averagePosition.X() << " " << averagePosition.Y() << " " << averagePosition.Z() << endl;
    ...
    if (last)
        // kb_debug << "===== final averagePosition: " << averagePosition.X() << "  " << averagePosition.Y() << "  " << averagePosition.Z() << endl;
        vertex -> SetPosition(averagePosition);

    return sTest;
}

void ATTPCVertexFindingTask::SetVertexPersistency(bool val) { fPersistency = val; }



void ATTPCVertexFindingTask::NewTrackWithHitClsuters(KBHit *vertex) // TODO for curling tracks
{
    kb_debug << endl;
    Int_t numTracks = fTrackArray -> GetEntriesFast();
    kb_debug << endl;
