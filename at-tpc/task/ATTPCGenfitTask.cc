...
ClassImp(ATTPCGenfitTask)

ATTPCGenfitTask::ATTPCGenfitTask()
:KBTask("ATTPCGenfitTask","")
{
}

bool ATTPCGenfitTask::Init()
{
    ...
    fBfieldX = par -> GetParDouble("bfieldX");
    fBfieldX = par -> GetParDouble("bfieldY");
    fBfieldX = par -> GetParDouble("bfieldZ");
    ...
    fMeasurementProducer = new genfit::MeasurementProducer<KBHit, genfit::ATTPCSpacepointMeasurement>(fGFTrackHitClusterArray);
    ...
    genfit::FieldManager::getInstance() -> init(new genfit::ConstField(fBfieldX, fBfieldY, 10*fBfieldZ)); //T -> kGauss
    cout << "ATTPCGenfitTask::genfit::FieldManager init " << fBfieldX << ", " << fBfieldY << ", " <<fBfieldZ << endl;

    genfit::MaterialEffects *materialEffects = genfit::MaterialEffects::getInstance();
    materialEffects -> init(new genfit::TGeoMaterialInterface());

    TVector3 posTarget(5., 5., 7.5); // cm
...
void ATTPCGenfitTask::Exec(Option_t*)
{
...
genfit::Track* ATTPCGenfitTask::FitTrack(KBHelixTrack *helixTrack, Int_t pdg)
{
    ...
    TVector3 momSeed = 0.001 * helixTrack -> Momentum(fBfieldX); // MeV -> GeV
    momSeed.SetTheta(dip); /// TODO

    kb_debug << "=====dip angle: " << dip << "  mom mag : " << momSeed.Mag() << "   mom x: " << momSeed.X()*1000 << "   mom Y: " << momSeed.Y()*1000 << "   mom Z: " << momSeed.Z()*1000<< endl;
    ...
    try { 
        ...
        // kb_debug << "==================== check the code fCurrentFitState ========================" << endl;
    }
    catch (genfit::Exception &e) { (genfit::Track *) nullptr; } /// TODO


    kb_debug << " fCurrentFitState.GetMom() "<<fCurrentFitState.getMom().Mag()*1000. <<  "   E -> "  << std::sqrt(fCurrentFitState.getMom().Mag2()*1000. + 938.272*938.272) << endl;

    try { 
        fCurrentTrackRep -> extrapolateToPlane(fCurrentFitState, fTargetPlane); 
        kb_debug << "==================== check the code extrapolateToPlane ========================" << endl;
