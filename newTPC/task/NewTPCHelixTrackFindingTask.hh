#ifndef NEWTPCHELIXTRACKFINDINGTASK_HH
#define NEWTPCHELIXTRACKFINDINGTASK_HH
...
class NewTPCHelixTrackFindingTask : public KBTask
{
  public:
    NewTPCHelixTrackFindingTask() : KBTask("NewTPCHelixTrackFindingTask","NewTPCHelixTrackFindingTask") {}
    virtual ~NewTPCHelixTrackFindingTask() {}
  ...
  private:
    ...
    Int_t fMinHitsToFitInitTrack = 7; ///< try track fit if track has more than this number of hits in track  // === finding from the distribution of hitted pad number. ===
    ...
    Int_t fCutMaxNumHitsInitTrack = 15; ///< return hits if track has more than this number of hits within initialization stage // === if select the gas type and the geometry, finding the mean hitted pad number +sigma with 1 track by event. 
    Int_t fCutMinNumHitsFinalTrack = 15; ///< remove track if track has smaller than this number of hits within initialization stage
    Double_t fCutMinHelixRadius = 30.; ///< helix radius cut for initialization stage       // === the minimum radius of the incident beam in TPC find to caculation at mixmum magnet. [mm]===
    ...
    Double_t fCutdkInExpectedTrackPath = 4.; // the correlation distance cut through helix axis during the track path between two hits [mm] // === use the CorrelateHitWithTrackCandidate() ===
  ...
  ClassDef(NewTPCHelixTrackFindingTask, 1)
};
