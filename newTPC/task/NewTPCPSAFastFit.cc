#include "NewTPCPSAFastFit.hh"

ClassImp(NewTPCPSAFastFit)

NewTPCPSAFastFit::NewTPCPSAFastFit()
: KBPulseGenerator()
{
  Init();
}

NewTPCPSAFastFit::NewTPCPSAFastFit(TString fileName)
: KBPulseGenerator(fileName)
{
  Init();
}

void NewTPCPSAFastFit::Init()
{
...
void 
NewTPCPSAFastFit::AnalyzeChannel(Double_t *buffer, vector<KBChannelHit> *hitArray, vector<vector<pair<double, double>>> *TBArray)
{
  Double_t adc[512] = {0};
  ...
  while (FindPeak(adc, /*get->*/tbPointer, tbStartOfPulse)) {
    ...
    vector<pair<double, double>> TBADCPad;
    ...
    if (TestPulse(adc, tbHitPre, amplitudePre, tbHit, amplitude, &TBADCPad)) 
    {

        TBArray -> push_back(TBADCPad);
...
Bool_t
NewTPCPSAFastFit::FindPeak(Double_t *adc, 
                          Int_t &tbPointer,
                          Int_t &tbStartOfPulse)
{
...
Bool_t
NewTPCPSAFastFit::FitPulse(Double_t *adc, 
                          Int_t tbStartOfPulse,
                          Int_t tbPeak,
                       Double_t &tbHit, 
                       Double_t &amplitude,
...
void 
NewTPCPSAFastFit::LSFitPulse(Double_t *buffer,
...
Bool_t
NewTPCPSAFastFit::TestPulse(Double_t *adc, 
                        Double_t tbHitPre,
                        ...
                        vector<pair<double, double>> *TBADCPad)
{
  ...
  for(int i=0; i<512; i++){
    double adcValue = adc[i];
    double fitADCValue = Pulse(i, amplitude, tbHit);
    TBADCPad->push_back(make_pair(adcValue, fitADCValue));
  }
